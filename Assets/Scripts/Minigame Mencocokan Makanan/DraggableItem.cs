...
public class DraggableItem : MonoBehaviour
{
    ...
    void Update()
    {
            ...
            switch (touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if (isDragging)
                    {
                        ...
                        Destroy(gameObject); // Destroy the food after dragging ends
                    }
                    break;
            }
        }
        ...
        if (Item_Display.isOriginalFoodClicked && !isDragging)
        {
            isDragging = true; // Start dragging immediately if the original food was clicked
            Item_Display.isOriginalFoodClicked = false; // Reset the flag after starting to drag
        }
    }
